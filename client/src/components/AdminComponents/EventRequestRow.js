  ...
  return (
    <>
      <tr className={styles.event_request_row}>
        ...
        <td>{startDate.getUTCHours()}:{(startDate.getUTCMinutes() < 10 && "0") + startDate.getUTCMinutes()} </td>
        <td>{endDate.getUTCHours()}:{(endDate.getUTCMinutes() < 10 && "0") + endDate.getUTCMinutes()} </td>
        <td>{data.venue}</td>
        <td>{!data.isApproved ? (
          <>
